...
This is the technical reference for the repo. For the pitch, see the site.

## Output preview

![Cook County food access map](analyses/demo-chicago-food-access/outputs/maps/food_access_composed.png)

*A composed food-access map generated by the pipeline from public datasets: census tracts are styled by poverty rate, point layers mark food resources, and callouts identify priority neighborhoods. Maps like this are produced as standard run artifacts, alongside style sidecars for GIS handoff.*

![HTML analysis report preview](docs/assets/readme-analysis-report.png)

*A self-contained HTML analysis report generated from the same project workspace, with an executive summary, QA status, key metrics, embedded maps, methods, caveats, and source notes.*
